...
**Новое:** `direction=balance_return` — возврат на личный баланс (пополнение без карты). В **детализации** приложения: название «Возврат на личный баланс», подпись категории «пополнение баланса». В **PDF-выписке**: название «Возврат на личный баланс», описание «основной баланс». Учитывается в `incomingTotal` и балансе так же, как `incoming`.
